...
const Navbar = () => {
  ...
  return (
    <>
      <div className='relative bg-[#eee] z-50 shadow-lg sticky top-0'>
        <nav className='container mx-auto flex items-center justify-between p-5'>
          <div>
            <Link to='/' className='text-blue-800 font-bold text-xl ml-5'>
            <FaBriefcase className="inline-block mr-1 mb-1 text-[#1177FF]" />Employment<span className='text-[#1177FF]'> Seeker.</span>
            </Link>
          </div>

          <ul className='hidden lg:flex gap-10 text-base text-6'>
            <li>
          ...
          <div className='hidden lg:block'>
            {!user?.token ? (
              <Link to='/user-auth'>
                <CustomButton
                  title='Sign In'
                  containerStyles='text-blue-600 py-1.5 px-5 focus:outline-none hover:bg-blue-700 hover:text-white rounded-md text-base border border-blue-600'
                />
              </Link>
            ) : (
              <div>
        ...
        <div
          className={`${
            isOpen ? "absolute flex bg-[#eee] " : "hidden"
